plotPvals = function(p, maxH)
{
	if(p < .01 & p >= .001)
	{
	        text(mean(centers), maxH+.25, labels="*")
	        arrows(centers[1],maxH,centers[2],maxH,code=3,lwd=1,angle=90,length=0.1,col="purple")
	}
	if(p < .001 & p >= .00001)
	{
	        text(mean(centers), maxH+.25, labels="**")
...
maxH=25
png(file="barrplot_NRF1ChIPSeq_SimilarityVfpkm_toSerum.png", height = 2000, width = 2000, res = 300)
...
centers=barplot(means, names.arg=c("< 1", ">= 1"), col="red", xlab="TRAP affinity", ylim=c(0,maxH+1.5), ylab="mean RPKM")
arrows(centers,means-ses,centers,means+ses,code=3,lwd=1.5,angle=90,length=0.15)
#title(main=paste("P=", formatC(p, format = "e", digits = 2), sep=""))
plotPvals(p,maxH)
dev.off()


png(file="histogram_NRF1ChIPSeq_sequenceSimilarities_toSerum.png", height = 2000, width = 2000, res = 300)
par(mar=c(4.8,4.6,1.35,1.2), cex.axis = 2, cex.main=2, cex.sub = 1.8, cex.lab = 2, mgp = c(3,1.1,0), mex = 1.5)
