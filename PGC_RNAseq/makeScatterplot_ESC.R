...
png(file="boxplots_ESCHivLo_SeqSimilarity.png", height = 2000, width = 2000, res = 300)
...
hi=hiData[hiData[,5] > 6,6]
nhi=length(hi)
mhi=mean(hi)
sehi=sd(hi)/sqrt(nhi)
means=c(mlow, mmed, mhi)
ses=c(selow,semed,sehi)
p1=t.test(med, low)$p.value
p2=t.test(med, hi)$p.value
p3=t.test(low, hi)$p.value

maxH=55
png(file="barrplot_ESCHi_SeqSimilarityVfpkm.png", height = 2000, width = 2000, res = 300)
par(mar=c(4.8,4.6,1.35,1.2), cex.axis = 2, cex.main=2, cex.sub = 1.8, cex.lab = 2, mgp = c(3,1.1,0), mex = 1.5)
#plot(hiData[,5], log(hiData[,6],2), xlim=c(0,25), xlab = paste("similarity index", sep = ""), ylab = "ESC DNAse-Hi FPKMs", col="red", main=cor(hiData[,5], log(hiData[,6],2)))
centers=barplot(means, names.arg=c("< 1", "1-6", "> 6"), col="red", xlab="TRAP affinity", ylim=c(0,maxH+6), ylab="mean RPKM")
arrows(centers,means-ses,centers,means+ses,code=3,lwd=magnify,angle=90,length=0.15)
plotPvals(p1,maxH-3,.4,centers[1],centers[2])
plotPvals(p2,maxH-1,.4,centers[2],centers[3])
plotPvals(p3,maxH+1,.4,centers[1],centers[3])
dev.off()
...
p3=t.test(low, hi)$p.value
maxH=5.5
png(file="barrplot_ESCAll_SeqSimilarityVfpkm.png", height = 2000, width = 2000, res = 300)
par(mar=c(4.8,4.6,1.35,1.2), cex.axis = 2, cex.main=2, cex.sub = 1.8, cex.lab = 2, mgp = c(3,1.1,0), mex = 1.5)
...
plotPvals(p1,maxH-.4,.16,centers[1],centers[2])
plotPvals(p2,maxH-.1,.16,centers[2],centers[3])
plotPvals(p3,maxH+.2,.16,centers[1],centers[3])
dev.off()

png(file="histogram_ESCHi_sequenceSimilarities.png", height = 2000, width = 2000, res = 300)
par(mar=c(4.8,4.6,1.35,1.2), cex.axis = 2, cex.main=2, cex.sub = 1.8, cex.lab = 2, mgp = c(3,1.1,0), mex = 1.5)
hist(hiData[,5], breaks=100, xlab="TRAP affinity", main="")
dev.off()

png(file="histogram_ESCAll_sequenceSimilarities.png", height = 2000, width = 2000, res = 300)
par(mar=c(4.8,4.6,1.35,1.2), cex.axis = 2, cex.main=2, cex.sub = 1.8, cex.lab = 2, mgp = c(3,1.1,0), mex = 1.5)
