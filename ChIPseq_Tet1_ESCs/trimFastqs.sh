...
do
	...
	summaryFile=$(basename $file .fastq.gz).txt
	echo $file2
	java -jar /Zulu/isaac/PGCme/Trimmomatic-0.38/trimmomatic-0.38.jar PE -threads 30 -summary $summaryFile $file $file2 $outFile s1 $outFile2 s2 ILLUMINACLIP:/Zulu/isaac/PGCme/Trimmomatic-0.38/adapters/all_PE.fa:1:0:2 TRAILING:20 MINLEN:20 &
done
wait
echo done
cd ..
