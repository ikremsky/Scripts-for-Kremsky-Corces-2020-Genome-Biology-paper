...
#x[which(x[,2] > 100),2]=100
#y[which(y[,2] > 100),2]=100
...
p=fisher.test(as.matrix(rbind(c(meth1, unmeth1), c(meth2, unmeth2))))$p.value
...
write.table(list(name, mean1, mean2, accessibleNum, inaccessibleNum, p, mean3), paste("avgme_", motifName, ".txt", sep=""), append=T, quote=F, sep="\t", row.names=F, col.names=F)
